...
class Settings extends Control {
  ...
  public changeInputsUpdate(car: ICar) {
    this.updateInputList.forEach((input) => { input.node.disabled = false; });
  ...
  private resetInputs(
    ...
    if (disable) this.updateInputList.forEach((input) => { input.node.disabled = true; });
  }

  private renderButton(isRace: boolean) {
    const buttonRace = new Button(this.buttonsControl.node, 'button', InnerButton.race, isRace);
