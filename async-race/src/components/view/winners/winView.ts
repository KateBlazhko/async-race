...
class WinView extends Control {
  ...
  private renderWinners(winners: IPageWinners) {
    ...
    this.list = winners.page.map((winner, index) => new Winner(
      this.winners.node,
      'winners__row',
      index + 1,
      winner,
  ...
  private renderHeader() {
    ...
    const cells = [
      new Control(row.node, 'div', 'winners__cell', InnerText.number),
      new Control(row.node, 'div', 'winners__cell', InnerText.car),
      new Control(row.node, 'div', 'winners__cell', InnerText.name),
    ];
    const wins = new Control(row.node, 'div', 'winners__cell winners__cell_sort', InnerText.wins);
    const time = new Control(row.node, 'div', 'winners__cell winners__cell_sort', InnerText.time);
